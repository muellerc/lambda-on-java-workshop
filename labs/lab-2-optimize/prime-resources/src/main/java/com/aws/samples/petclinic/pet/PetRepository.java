...
public class PetRepository {
    ...
    public void prime() {
        try {
            ...
            item.put("name", AttributeValue.builder().s("Max").build());
            item.put("type", AttributeValue.builder().s("cat").build());
            item.put("birthday", AttributeValue.builder().s("2016-03-09").build());

            // will fail because of the missing partition key
            dynamoDbClient.putItem(PutItemRequest.builder()
                    .tableName(table)
                    .item(item)
                    .build());
